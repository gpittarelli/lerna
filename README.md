...
When run with this flag, `publish` will update the dependencies of updated packages with the new version, except the new version is specified exactly, instead of as semver compatible (with a `^`).
